...
// Default export, to make the module compatible with ES6-style imports
...
/*
 * LeafletTimeDimension.TimeDimension: TimeDimension object manages the time component of a layer.
...
LeafletTimeDimension.TimeDimension = (L.Layer || L.Class).extend({
    ...
    _generateAvailableTimes: function () {
        if (this.options.times) {
            return LeafletTimeDimension.TimeDimension.Util.parseTimesExpression(this.options.times);
        } else if (this.options.timeInterval) {
            var tiArray = LeafletTimeDimension.TimeDimension.Util.parseTimeInterval(this.options.timeInterval);
            var period = this.options.period || 'P1D';
            var validTimeRange = this.options.validTimeRange || undefined;
            return LeafletTimeDimension.TimeDimension.Util.explodeTimeRange(tiArray[0], tiArray[1], period, validTimeRange);
        } else {
            return [];
        }
    },
    ...
    setAvailableTimes: function (times, mode) {
        ...
        if (mode == 'extremes') {
            var period = this.options.period || 'P1D';
            this._availableTimes = LeafletTimeDimension.TimeDimension.Util.explodeTimeRange(new Date(times[0]), new Date(times[times.length - 1]), period);
        } else {
            var parsedTimes = LeafletTimeDimension.TimeDimension.Util.parseTimesExpression(times);
            if (this._availableTimes.length === 0) {
                this._availableTimes = parsedTimes;
            } else if (mode == 'intersect') {
                this._availableTimes = LeafletTimeDimension.TimeDimension.Util.intersect_arrays(parsedTimes, this._availableTimes);
            } else if (mode == 'union') {
                this._availableTimes = LeafletTimeDimension.TimeDimension.Util.union_arrays(parsedTimes, this._availableTimes);
...
LeafletTimeDimension.timeDimension = function (options) {
    return new LeafletTimeDimension.TimeDimension(options);
};
...
/*
 * LeafletTimeDimension.TimeDimension.Layer:  an abstract Layer that can be managed/synchronized with a TimeDimension.
...
LeafletTimeDimension.TimeDimension.Layer = (L.Layer || L.Class).extend({
...
LeafletTimeDimension.timeDimension.layer = function(layer, options) {
    return new LeafletTimeDimension.TimeDimension.Layer(layer, options);
};

/*
 * LeafletTimeDimension.TimeDimension.Layer.WMS: wms Layer associated to a TimeDimension
 */

LeafletTimeDimension.TimeDimension.Layer.WMS = LeafletTimeDimension.TimeDimension.Layer.extend({

    initialize: function(layer, options) {
        LeafletTimeDimension.TimeDimension.Layer.prototype.initialize.call(this, layer, options);
    ...
    eachLayer: function(method, context) {
        ...
        return LeafletTimeDimension.TimeDimension.Layer.prototype.eachLayer.call(this, method, context);
    },
    ...
    onAdd: function(map) {
        LeafletTimeDimension.TimeDimension.Layer.prototype.onAdd.call(this, map);
        if (this._availableTimes.length == 0) {
            this._requestTimeDimensionFromCapabilities();
        } else {
    ...
    setOpacity: function(opacity) {
        LeafletTimeDimension.TimeDimension.Layer.prototype.setOpacity.apply(this, arguments);
    ...
    setZIndex: function(zIndex){
        LeafletTimeDimension.TimeDimension.Layer.prototype.setZIndex.apply(this, arguments);
        // apply to all preloaded caches
        for (var prop in this._layers) {
...
LeafletTimeDimension.timeDimension.layer.wms = function(layer, options) {
    return new LeafletTimeDimension.TimeDimension.Layer.WMS(layer, options);
};

/*
 * LeafletTimeDimension.TimeDimension.Layer.GeoJson:
 */

LeafletTimeDimension.TimeDimension.Layer.GeoJson = LeafletTimeDimension.TimeDimension.Layer.extend({
    ...
    _update: function() {
        ...
        if (this._duration) {
            ...
            LeafletTimeDimension.TimeDimension.Util.subtractTimeDuration(date, this._duration, true);
    ...
    _setAvailableTimes: function() {
        ...
        this._availableTimes = LeafletTimeDimension.TimeDimension.Util.sort_and_deduplicate(times);
...
LeafletTimeDimension.timeDimension.layer.geoJson = function(layer, options) {
    return new LeafletTimeDimension.TimeDimension.Layer.GeoJson(layer, options);
};

/*
 * LeafletTimeDimension.TimeDimension.Player
 */
LeafletTimeDimension.TimeDimension.Player = (L.Layer || L.Class).extend({
...
LeafletTimeDimension.UI.Knob = L.Draggable.extend({
    options: {
        className: 'knob',
        step: 1,
        rangeMin: 0,
...
/*
 * LeafletTimeDimension.Control.TimeDimension: Leaflet control to manage a timeDimension
 */
LeafletTimeDimension.Control.TimeDimension = L.Control.extend({
    options: {
    ...
    _initPlayer: function() {
        if (!this._player){ // in case of remove/add
            if (this.options.player) {
                ...
                this._player = new LeafletTimeDimension.TimeDimension.Player(this.options.playerOptions, this._timeDimension);
            }
        }
    ...
    _createSliderTime: function(className, container) {
        ...
        knob = new LeafletTimeDimension.UI.Knob(sliderbar, {
            className: 'knob main',
            rangeMin: 0,
            rangeMax: max
        });
    ...
    _createLimitKnobs: function(sliderbar) {
        ...
        var lknob = new LeafletTimeDimension.UI.Knob(sliderbar, {
            className: 'knob lower',
            rangeMin: 0,
            rangeMax: max
        });
        var uknob = new LeafletTimeDimension.UI.Knob(sliderbar, {
...
*/
        ...
        var knob = new LeafletTimeDimension.UI.Knob(sliderbar, {
...
LeafletTimeDimension.control.timeDimension = function(options) {
    return new LeafletTimeDimension.Control.TimeDimension(options);
};

export default LeafletTimeDimension;
